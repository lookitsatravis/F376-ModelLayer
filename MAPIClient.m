...
- (id)initWithBaseURL:(NSURL *)url
{
    ...
    if (self) {
        ...
        [self setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus1 status) {
        ...
        [[AFNetworkActivityIndicatorManager1 sharedManager] setEnabled:YES];
    }
    return self;
}

- (void)apiReachabilityChanged:(AFNetworkReachabilityStatus1)status
{
    if (status == AFNetworkReachabilityStatusNotReachable1) {
...
- (void)queueAPITransaction:(MAPITransaction*)a
{
    ...
    if (([self networkReachabilityStatus] != AFNetworkReachabilityStatusNotReachable1) && (![a started]))
        [a performDeferred];
...
{
    if ([self networkReachabilityStatus] != AFNetworkReachabilityStatusNotReachable1)
        [self performNextAction];
    else {
...
//        [[[UIAlertView alloc] initWithTitle:@"Error" message:message delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] show];
        ...
        UIColor *color = [UIColor colorWithRed:229.0/255.0 green:125.0/255.0 blue:54.0/255.0 alpha:1];
